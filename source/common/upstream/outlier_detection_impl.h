...
public:
  ...
  void resetConsecutiveConnectionFailure() { consecutive_connection_failure_ = 0; }
...
private:
  ...
  std::atomic<uint32_t> consecutive_connection_failure_{0};
...
#define ALL_OUTLIER_DETECTION_STATS(COUNTER, GAUGE)                                                \
  ...
  COUNTER(ejections_consecutive_connection_failure)                                                \
...
public:
  ...
  uint64_t consecutiveConnectionFailure() { return consecutive_connection_failure_; }
  ...
  uint64_t enforcingConsecutiveConnectionFailure() { return enforcing_consecutive_connection_failure_; }
...
private:
  ...
  const uint64_t consecutive_connection_failure_;
  ...
  const uint64_t enforcing_consecutive_connection_failure_;
...
public:
  ...
  void onConsecutiveConnectionFailure(HostSharedPtr host);
...
private:
  ...
  void onConsecutiveConnectionFailureWorker(HostSharedPtr host);
  void notifyWorkerConsecutiveError(HostSharedPtr host, std::function<void(std::shared_ptr<DetectorImpl>, HostSharedPtr)> f);
