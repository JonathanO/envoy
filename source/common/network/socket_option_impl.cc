...
int SocketOptionImpl::setIpSocketOption(Socket& socket, SocketOptionName ipv4_optname,
  ...
  if (ip->version() == Network::Address::IpVersion::v4) {
    ...
    return os_syscalls.setsockopt(socket.fd(), ipv4_optname.value().first, ipv4_optname.value().second, optval, optlen);
  }
  ...
  if (ipv6_optname) {
    return os_syscalls.setsockopt(socket.fd(), ipv6_optname.value().first, ipv6_optname.value().second, optval, optlen);
  }
  if (ipv4_optname) {
    return os_syscalls.setsockopt(socket.fd(), ipv4_optname.value().first, ipv4_optname.value().second, optval, optlen);
  }
