...
int TcpKeepaliveOptionImpl::setSocketOption(Socket& socket,
...
int TcpKeepaliveOptionImpl::setSocketOption(Socket& socket, Network::SocketOptionName optname, int value) {
  if (!optname.has_value()) {
    errno = ENOTSUP;
    return -1;
  }
  auto& os_syscalls = Api::OsSysCallsSingleton::get();
  return os_syscalls.setsockopt(socket.fd(), optname.value().first, optname.value().second, &value, sizeof(value));
}
